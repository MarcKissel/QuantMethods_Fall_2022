...
- it is sorta like an interactive R session that user is running but often they dont know it...it takes inputs from user and runs functions to get outputs
...
reactive output- this is part of the UI that displays the content  that the server produces...so it could be a map that is created behing the scenes  and then the user sees
...
Server - this defines and processes the data that will be displayed in the UI. Server for shiny app is defined as a function (in contrast to the UI which is a value)..function needs to take 2 lists of arguments, input and ouput. the input comes from the user via user inference and are used to create content. This content is saved in the output list so that it can be sent back to the UI to be used in the browser....the server uses RENDER FUNCTIONS to assign these values to output so that the content will automatically be recalculated whenever the input list changes



WHEN we hit run, shiny combines the UI and Serve component into a webpage
